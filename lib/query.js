...
exports.FilterStatement = FilterStatement;
class Query {
    constructor(fields) {
        this.fields = null;
        this.idx = 'MASTER';
    ...
    aggregate(field, aggregationType, size = 10) {
        field = this.prefixGolden(field);
        const fieldParts = field.split('.');
        let aggregationRoot = {};
        let aggregation = aggregationRoot;
    ...
    execute() {
        if (!this.namedQuery) {
            ...
            return http_client_1.httpClient.post(url, query, queryParams).then((response) => {
            ...
            return http_client_1.httpClient.post(url, this.namedQueryParams, queryParams);
        }
    }
    deepFind(obj, path) {
